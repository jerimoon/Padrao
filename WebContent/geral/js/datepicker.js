...
//		this._div.innerHTML = '<table><thead><tr><th width="10px" id="'+ this._id_datepicker_prev +'" style="cursor: pointer;">&nbsp;&lt;&lt;&nbsp;</th><th id="'+ this._id_datepicker_hdr +'" colspan="5"></th><th width="10px" id="'+ this._id_datepicker_next +'" style="cursor: pointer;">&nbsp;&gt;&gt;&nbsp;</th></tr></thead><tbody id="'+ this._id_datepicker +'-tbody"></tbody><tfoot><td colspan="7" id="'+ this._id_datepicker_ftr +'"></td></tfoot></table>';
  		...
  		var today = new Element('a',{href:'javascript:setToday("'+this._relative+'")',style:"vertical-align: super;",id:this._relative+"_settoday",name:this._relative+"_settoday"});
  		today.appendChild(document.createTextNode(" Hoje"));
  		var divdate = Builder.node('div',{className : "divdate",style:"vertical-align: middle;"}); 
  		
		/* Build the datepicker icon */
		var datepickeropener = Builder.node('table',{className : "datepicker-opener-table",style:'display:inline-table;'});
		divdate.appendChild(datepickeropener);
		divdate.appendChild(today);
		...
		this.insertAfter($(this._relative).parentNode,divdate,$(this._relative));
...
function setToday(id){
	...
	$(id).value =  day+'/'+month+'/'+today.getFullYear();
}
function setNow(id){
	var now =new Date();
